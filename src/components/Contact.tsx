...
const Contact = () => {
  ...
  return (
    <section id="contact" className="section-padding bg-black overflow-hidden relative" style={{ backgroundColor: '#000000' }}>
      ...
      <div className="container-custom relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-12 xl:gap-16">
          {/* Contact Information */}
          <div className="space-y-8 xl:space-y-10 animate-slide-up delay-100">
            ...
            <div className="space-y-4 xl:space-y-5">
              {contactInfo.map((info, index) => (
                <div key={index} className="flex items-center gap-4 xl:gap-5 animate-fade-in" style={{ animationDelay: `${index * 120}ms` }}>
                  <div className="w-10 h-10 xl:w-12 xl:h-12 bg-primary/10 rounded-lg flex items-center justify-center shadow-orange-glow animate-icon-bounce">
                    <info.icon className="text-primary w-5 h-5 xl:w-6 xl:h-6" />
                  </div>
                  <div>
                    <p className="font-medium text-base xl:text-lg 2xl:text-xl">{info.label}</p>
                    {info.link ? (
                      <a 
                        href={info.link}
                        className="text-muted-foreground hover:text-primary transition-colors text-sm xl:text-base 2xl:text-lg"
                      >
                        {info.value}
                      </a>
                    ) : (
                      <p className="text-muted-foreground text-sm xl:text-base 2xl:text-lg">{info.value}</p>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-4">
              <h4 className="font-semibold text-base xl:text-lg 2xl:text-xl">Follow Me</h4>
              <div className="flex gap-4 xl:gap-5">
                {socialLinks.map((social, index) => (
                  ...
                  >
                    <Card className="card-elegant w-16 h-16 xl:w-20 xl:h-20 group-hover:shadow-orange-glow group-hover:scale-110 transition-all duration-300">
                      <CardContent className="p-0 flex items-center justify-center h-full">
                        <social.icon 
                          className="text-muted-foreground group-hover:text-primary transition-colors w-5 h-5 xl:w-6 xl:h-6" 
                        />
                      </CardContent>
                    </Card>
                  </a>
              ...
              <div className="space-y-1 text-sm xl:text-base 2xl:text-lg text-muted-foreground">
                {socialLinks.map((social, index) => (
          ...
          >
            <Card className="card-elegant animate-slide-up delay-200">
              <CardContent className="p-6 xl:p-8">
                <form action="https://formspree.io/f/mblkdwoj" method="POST" className="space-y-6 xl:space-y-8">
                  <div className="space-y-2">
                    <h3 className="text-xl xl:text-2xl 2xl:text-3xl font-semibold">Send me a message</h3>
                  ...
                  <div className="space-y-4 xl:space-y-5">
                    <div className="animate-fade-in">
                      <label htmlFor="name" className="block text-sm xl:text-base font-medium mb-2">
                        Your Name
                      </label>
                      <Input
                        id="name"
